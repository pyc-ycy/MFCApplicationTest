...
BEGIN_MESSAGE_MAP(CMFCApplication1Doc, CDocument)
	...
	//{{AFX_MSG_MAP(CMFCApplication1Doc)
	ON_COMMAND(ID_TEST_IN,OnMenuitemsettitle)  //消息映射
	ON_COMMAND(ID_MYDEBUG, OnDeBug)
	//}}AFX_MSG
END_MESSAGE_MAP()
...
//定义设置标题命令处理函数
void CMFCApplication1Doc::OnMenuitemsettitle()
{
	//设置标题
	SetTitle(CTime::GetCurrentTime().Format("log %Y-%m-%d  %H:%M:%S"));
}
void CMFCApplication1Doc::OnDeBug()
{
	SetTitle(L"DeBug");
}
